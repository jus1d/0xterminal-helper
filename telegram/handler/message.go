...
func (h *Handler) TextMessage(u tgbotapi.Update) {
	...
	if exists && stage == WaitingWordList {
		...
		h.sendTextMessage(userID, content, nil)
	} else if exists && stage == WaitingAttempt {
		parts := strings.Split(u.Message.Text, " ")
		if len(parts) < 2 {
			h.sendTextMessage(userID, "U invalid. Use: word guessed-letters-amount", nil)
			return
		}
		word := parts[0]
		guessedLetters, err := strconv.Atoi(parts[1])
		if err != nil {
			h.sendTextMessage(userID, "Guessed letters amount should be integer. Use: word guessed-letters-amount", nil)
			return
		}
